...
public class TBorrower implements MessageListener {
	...
	public TBorrower(String topiccf, String topicName, String rate) {
		try {
			...
			//durable
			//TopicSubscriber subscriber1 = tSession.createDurableSubscriber(topic, "name");
	...
	public static void main(String argv[]) {
		...
		if (argv.length == 3) {
			...
			topicName = argv[1];
			rate = argv[2];
			borrower = new TBorrower(topiccf, topicName, rate);
		} else {
			borrower = new TBorrower("TopicCF", "RateTopic", "5.6");
		}
