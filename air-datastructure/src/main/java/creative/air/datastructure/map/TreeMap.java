...
public class TreeMap<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V>, Cloneable, java.io.Serializable {
	...
	/**
	 * Constructs a new, empty tree map, using the natural ordering of its keys. All keys
	 * inserted into the map must implement the {@link Comparable} interface. Furthermore, all
	 * such keys must be <i>mutually comparable</i>: <tt>k1.compareTo(k2)</tt> must not throw a
	 * <tt>ClassCastException</tt> for any keys <tt>k1</tt> and <tt>k2</tt> in the map. If the
	 * user attempts to put a key into the map that violates this constraint (for example, the
	 * user attempts to put a string key into a map whose keys are integers), the
	 ...
	 * inserted into the map must be <i>mutually comparable</i> by the given comparator:
	 * <tt>comparator.compare(k1,
	 * k2)</tt> must not throw a <tt>ClassCastException</tt> for any keys <tt>k1</tt> and
	 * <tt>k2</tt> in the map. If the user attempts to put a key into the map that violates this
	 * constraint, the <tt>put(Object
	 ...
	 * according to the <i>natural ordering</i> of its keys. All keys inserted into the new map
	 * must implement the {@link Comparable} interface. Furthermore, all such keys must be
	 * <i>mutually comparable</i>: <tt>k1.compareTo(k2)</tt> must not throw a
	 * <tt>ClassCastException</tt> for any keys <tt>k1</tt> and <tt>k2</tt> in the map. This
	 ...
	 * 
	 * @param m
	 * the sorted map whose mappings are to be placed in this map, and whose comparator is to be
	 ...
	 * @throws NullPointerException
	 * if the specified key is null and this map uses natural ordering, or its comparator does
	 ...
	 * formally, returns <tt>true</tt> if and only if this map contains at least one mapping to
	 * a value <tt>v</tt> such that <tt>(value==null ? v==null : value.equals(v))</tt>. This
	 * operation will probably require time linear in the map size for most implementations.
	 * 
	 * @param value
	 ...
	 * such that {@code key} compares equal to {@code k} according to the map's ordering, then
	 * this method returns {@code v}; otherwise it returns {@code null}. (There can be at most
	 * one such mapping.)
	 * <p>
	 * A return value of {@code null} does not <i>necessarily</i> indicate that the map contains
	 ...
	 * map
	 * @throws NullPointerException
	 * if the specified map is null or the specified map contains a null key and this map does
	 * not permit null keys
	 */
	...
	/**
	 * Returns this map's entry for the given key, or <tt>null</tt> if the map does not contain
	 * an entry for the key.
	 * 
	 * @return this map's entry for the given key, or <tt>null</tt> if the map does not contain
	 * an entry for the key
	 * @throws ClassCastException
	 * if the specified key cannot be compared with the keys currently in the map
	 * @throws NullPointerException
	...
	/**
	 * Version of getEntry using comparator. Split off from getEntry for performance. (This is
	 * not worth doing for most methods, that are less dependent on comparator performance, but
	 * is worthwhile here.)
	 */
	...
	/**
	 * Gets the entry corresponding to the specified key; if no such entry exists, returns the
	 * entry for the least key greater than the specified key; if no such entry exists (i.e.,
	 * the greatest key in the Tree is less than the specified key), returns <tt>null</tt>.
	 */
	 ...
	 * returns the entry for the least key greater than the specified key; if no such entry
	 * exists returns <tt>null</tt>.
	 */
	...
	/**
	 * Returns a shallow copy of this <tt>TreeMap</tt> instance. (The keys and values themselves
	 * are not cloned.)
	 * 
	...
	/**
	 * Fields initialized to contain an instance of the entry set view the first time this view
	 * is requested. Views are stateless, so there's no reason to create more than one.
	 */
	...
	/**
	 * Returns a {@link Set} view of the keys contained in this map. The set's iterator returns
	 * the keys in ascending order. The set is backed by the map, so changes to the map are
	 * reflected in the set, and vice-versa. If the map is modified while an iteration over the
	 * set is in progress (except through the iterator's own <tt>remove</tt> operation), the
	 * results of the iteration are undefined. The set supports element removal, which removes
	 * the corresponding mapping from the map, via the <tt>Iterator.remove</tt>,
	 * <tt>Set.remove</tt>, <tt>removeAll</tt>, <tt>retainAll</tt>, and <tt>clear</tt>
	 ...
	 * iterator returns the values in ascending order of the corresponding keys. The collection
	 ...
	 * (except through the iterator's own <tt>remove</tt> operation), the results of the
	...
	/**
	 * Returns a {@link Set} view of the mappings contained in this map. The set's iterator
	 * returns the entries in ascending key order. The set is backed by the map, so changes to
	 * the map are reflected in the set, and vice-versa. If the map is modified while an
	 * iteration over the set is in progress (except through the iterator's own <tt>remove</tt>
	 * operation, or through the <tt>setValue</tt> operation on a map entry returned by the
	 ...
	 * {@inheritDoc}
	 * @throws NullPointerException
	 * if <tt>fromKey</tt> or <tt>toKey</tt> is null and this map uses natural ordering, or its
	 ...
	 * {@inheritDoc}
	 * @throws NullPointerException
	 * if <tt>toKey</tt> is null and this map uses natural ordering, or its comparator does not
	 * permit null keys
	 * @throws IllegalArgumentException
	 * {@inheritDoc}
	 * @since 1.6
	 ...
	 * {@inheritDoc}
	 * @throws NullPointerException
	 * if <tt>fromKey</tt> is null and this map uses natural ordering, or its comparator does
	 * not permit null keys
	 * @throws IllegalArgumentException
	 * {@inheritDoc}
	 * @since 1.6
	...
	/*
	 * Unlike Values and EntrySet, the KeySet class is static, delegating to a NavigableMap to
	 * allow use by SubMaps, which outweighs the ugliness of needing type-tests for the
	 * following Iterator methods that are defined appropriately in main versus submap classes.
	 */
	...
	static abstract class NavigableSubMap<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V>, java.io.Serializable {
		 ...
		 * backing map, and the other values are ignored. Otherwise, if loInclusive is true, lo
		 * is the inclusive bound, else lo is the exclusive bound. Similarly for the upper
		 * bound.
		 */
		final K lo, hi;
		...
		/*
		 * Absolute versions of relation operations. Subclasses map to these using like-named
		 * "sub" versions that invert senses for descending maps
		 */
	...
	/**
	 * This class exists solely for the sake of serialization compatibility with previous
	 * releases of TreeMap that did not support NavigableMap. It translates an old-version
	 * SubMap into a new-version AscendingSubMap. This class is never otherwise used.
	 * 
	 * @serial include
	 */
	...
	/**
	 * Node in the Tree. Doubles as a means to pass key-value pairs back to user (see
	 * Map.Entry).
	 */
	...
	static final class Entry<K, V> implements Map.Entry<K, V> {
		...
		/**
		 * Make a new cell with given key, value, and parent, and with <tt>null</tt> child
	 ...
	 * slightly different than the CLR version. Rather than using dummy nilnodes, we use a set
	 * of accessors that deal properly with null. They are used to avoid messiness surrounding
	 * nullness checks in the main algorithms.
	 */
	 ...
	 * 
	 * @serialData The <i>size</i> of the TreeMap (the number of key-value mappings) is emitted
	 ...
	 * determined by the TreeMap's Comparator, or by the keys' natural ordering if the TreeMap
	 * has no Comparator).
	 */
	 ...
	 * iterator or stream. This leads to too many parameters, but seems better than
	 * alternatives. The four formats that this method accepts are: 1) An iterator of
	 * Map.Entries. (it != null, defaultVal == null). 2) An iterator of keys. (it != null,
	 * defaultVal != null). 3) A stream of alternating serialized keys and values. (it == null,
	 * defaultVal == null). 4) A stream of serialized keys. (it == null, defaultVal != null). It
	 * is assumed that the comparator of the TreeMap is already set prior to calling this
	 * method.
	 * 
	 * @param size
	 ...
	 * @param str
	 * If non-null, new entries are created from keys and possibly values read from this stream
	 * in serialized form. Exactly one of it and str should be non-null.
	 * @param defaultVal
	 * if non-null, this default value is used for each value in the map. If null, each value is
	 * read from iterator or stream, as described above.
	 * @throws IOException
	 ...
	 * named parameters have identical definitions. Additional parameters are documented below.
	 * It is assumed that the comparator and size fields of the TreeMap are already set prior to
	 ...
	 * @param redLevel
	 * the level at which nodes should be red. Must be equal to computeRedLevel for tree of this
	 * size.
	 */
	private final Entry<K, V> buildFromSorted(int level, int lo, int hi, int redLevel, Iterator it,
		 ...
		 * recursively construct the entire left subtree, so as to grab all of its elements. We
		 * can then proceed with right subtree. The lo and hi arguments are the minimum and
		 * maximum indices to pull out of the iterator or stream for current subtree. They are
		 * not actually indexed, we just proceed sequentially, ensuring that items are extracted
	...
	/**
	 * Find the level down to which to assign all nodes BLACK. This is the last `full' level of
	 * the complete binary tree produced by buildTree. The remaining nodes are colored RED.
	 * (This makes a `nice' set of color assignments wrt future insertions.) This level number
	 * is computed by finding the number of splits needed to reach the zeroeth node. (The answer
