...
public abstract class AbstractMap<K, V> implements Map<K, V> {
	 ...
	 * <tt>getValue</tt> operation, the entry is removed from the collection (and the backing
	 * map) with the iterator's <tt>remove</tt> operation, and the saved value is returned. If
	 * the iteration terminates without finding such an entry, <tt>null</tt> is returned. Note
	 ...
	 * <tt>entrySet</tt> iterator does not support the <tt>remove</tt> method and this map
	 ...
	 * <p>
	 * Note that this implementation throws an <tt>UnsupportedOperationException</tt> if this
	 * map does not support the <tt>put</tt> operation and the specified map is nonempty.
	 * 
	 * @throws UnsupportedOperationException
	 * {@inheritDoc}
	 * @throws ClassCastException
	...
	/**
	 * Each of these fields are initialized to contain an instance of the appropriate view the
	 * first time this view is requested. The views are stateless, so there's no reason to
	 * create more than one of each.
	 */
	 ...
	 * {@inheritDoc}
	 * <p>
	 * This implementation returns a set that subclasses {@link AbstractSet}. The subclass's
	 * iterator method returns a "wrapper object" over this map's <tt>entrySet()</tt> iterator.
	 * The <tt>size</tt> method delegates to this map's <tt>size</tt> method and the
	 ...
	 * subsequent calls. No synchronization is performed, so there is a slight chance that
	 ...
	 * <p>
	 * The collection is created the first time this method is called, and returned in response
	 * to all subsequent calls. No synchronization is performed, so there is a slight chance
	 * that multiple calls to this method will not all return the same collection.
	 */
	...
	/**
	 * Compares the specified object with this map for equality. Returns <tt>true</tt> if the
	 * given object is also a map and the two maps represent the same mappings. More formally,
	 * two maps <tt>m1</tt> and <tt>m2</tt> represent the same mappings if
	 * <tt>m1.entrySet().equals(m2.entrySet())</tt>. This ensures that the <tt>equals</tt>
	 * method works properly across different implementations of the <tt>Map</tt> interface.
	 * <p>
	 * This implementation first checks if the specified object is this map; if so it returns
	 * <tt>true</tt>. Then, it checks if the specified object is a map whose size is identical
	 * to the size of this map; if not, it returns <tt>false</tt>. If so, it iterates over this
	 * map's <tt>entrySet</tt> collection, and checks that the specified map contains each
	 ...
	 * of the hash codes of each entry in the map's <tt>entrySet()</tt> view. This ensures that
	 * <tt>m1.equals(m2)</tt> implies that <tt>m1.hashCode()==m2.hashCode()</tt> for any two
	 * maps <tt>m1</tt> and <tt>m2</tt>, as required by the general contract of
	 ...
	 * of key-value mappings in the order returned by the map's <tt>entrySet</tt> view's
	 * iterator, enclosed in braces (<tt>"{}"</tt>). Adjacent mappings are separated by the
	 * characters <tt>", "</tt> (comma and space). Each key-value mapping is rendered as the key
	 * followed by an equals sign (<tt>"="</tt>) followed by the associated value. Keys and
	...
	/**
	 * Returns a shallow copy of this <tt>AbstractMap</tt> instance: the keys and values
	...
	/**
	 * Utility method for SimpleEntry and SimpleImmutableEntry. Test for equality, checking for
	 * nulls.
	 */
	 ...
	 * <tt>setValue</tt> method. This class facilitates the process of building custom map
	...
	public static class SimpleEntry<K, V> implements Entry<K, V>, java.io.Serializable {
		...
		/**
		 * Compares the specified object with this entry for equality. Returns {@code true} if
		 * the given object is also a map entry and the two entries represent the same mapping.
		 ...
		 * {@code e1.hashCode()==e2.hashCode()} for any two Entries {@code e1} and {@code e2},
		 ...
		 * string representation of this entry's key followed by the equals character ("
		 * <tt>=</tt>") followed by the string representation of this entry's value.
		 * 
	...
	public static class SimpleImmutableEntry<K, V> implements Entry<K, V>, java.io.Serializable {
		 ...
		 * operation). This implementation simply throws <tt>UnsupportedOperationException</tt>,
		 * as this class implements an <i>immutable</i> map entry.
		 * 
		 * @param value
		 * new value to be stored in this entry
